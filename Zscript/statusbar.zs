...
class HDStatusBar:DoomStatusBar{
	...
	void DrawCommonStuff(bool usemughud){
		...
		if(hudlevel==2){
			...
			if(hpl.enc){
				...
				drawstring(
					...
					(8,mxht),DI_TEXT_ALIGN_LEFT|DI_SCREEN_LEFT_CENTER,
				...
				fill(
					color(128,96,96,96),
					4,encbarheight,1,-1,
					DI_SCREEN_LEFT_CENTER|DI_ITEM_LEFT
				);
				fill(
					color(128,96,96,96),
					5,encbarheight,1,-20,
		...
		if(usemughud)DrawTexture(
			GetMugShot(5,Mugshot.CUSTOM,getmug(hpl.mugshot)),(0,0),
		...
		for(item=cplayer.mo.inv;item!=NULL;item=item.inv){
			...
			if(drawfull&&hudlevel==2){
				...
				drawtexture(icon,
					...
					DI_ITEM_CENTER_BOTTOM|DI_SCREEN_RIGHT_CENTER
					|((
						(ivsh&&ivsh.bdroptranslation)
						||(ivsw&&ivsw.bdroptranslation)
					)?DI_TRANSLATABLE:0),
	...
	void drawammocounters(int mxht){
		...
		for(int i=0;i<ammosprites.size();i++){
			...
			drawimage(ammosprites[i],
				...
				DI_SCREEN_RIGHT_CENTER|DI_ITEM_RIGHT_BOTTOM,
				//alpha:count?1.:0.2,
			...
			drawstring(
				...
				DI_SCREEN_RIGHT_CENTER|DI_ITEM_RIGHT_BOTTOM|DI_TEXT_ALIGN_RIGHT,
